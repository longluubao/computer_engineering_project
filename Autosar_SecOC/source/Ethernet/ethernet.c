...
Std_ReturnType EthDrv_Send(unsigned short id, unsigned char* data , uint16 dataLen) {
    ...
    {
        #ifdef ETHERNET_DEBUG
            (void)printf("Create Socket Error\n");
        #endif
        return E_NOT_OK;
    }
...
Std_ReturnType EthDrv_Receive(unsigned char* data , uint16 dataLen, unsigned short* id, uint16* actualSize)
{
    ...
    #ifdef ETHERNET_DEBUG
        (void)printf("######## in Recieve Ethernet\n");
    #endif
    /* create a socket*/
    int server_socket;
    int client_socket;
    ...
    {
        #ifdef ETHERNET_DEBUG
            (void)printf("connect setsockopt Error \n");
        #endif
        return E_NOT_OK;
