...
│     OQS_SIG_sign → 3309-byte signature                        │  │  113.16 µs
└───────────────────────────────┬───────────────────────────────┘ ─┘
                                ▼
...
│         → CanIf_Transmit(fragment[i])                         │ │    2.02 µs
└───────────────────────────────┬───────────────────────────────┘ ─┘
                                ▼                                ─┐
┌───────────────────────────────────────────────────────────────┐ │
...
End-to-end (app Tx → app Rx): 181.71 µs
```
...
|----------------------------------------------|---------------|-------------|------|
| Com + PduR + SecOC header + FVM (residual)    |          0.34 | ≤       200 | PASS |
| Csm + CryIf + PQC + liboqs sign               |        113.16 | ≤       500 | PASS |
| CanTp / SoAd fragmentation                    |          2.02 | ≤       300 | PASS |
| CanIf / EthIf / driver + physical bus transit |         66.18 | ≤      2000 | PASS |
| Csm + CryIf + PQC + liboqs verify             |          0.00 | ≤       500 | PASS |
| End-to-end (application → application)        |        181.71 | ≤      5000 | PASS |
