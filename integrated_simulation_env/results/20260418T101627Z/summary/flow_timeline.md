...
│     OQS_SIG_sign → 3309-byte signature                        │  │  189.45 µs
└───────────────────────────────┬───────────────────────────────┘ ─┘
                                ▼
...
│         → CanIf_Transmit(fragment[i])                         │ │    2.11 µs
└───────────────────────────────┬───────────────────────────────┘ ─┘
                                ▼                                ─┐
┌───────────────────────────────────────────────────────────────┐ │
...
End-to-end (app Tx → app Rx): 338.20 µs
```
...
| Csm + CryIf + PQC + liboqs sign               |        189.45 | ≤       500 | PASS |
| CanTp / SoAd fragmentation                    |          2.11 | ≤       300 | PASS |
| CanIf / EthIf / driver + physical bus transit |        146.25 | ≤      2000 | PASS |
| Csm + CryIf + PQC + liboqs verify             |          0.00 | ≤       500 | PASS |
| End-to-end (application → application)        |        338.20 | ≤      5000 | PASS |
