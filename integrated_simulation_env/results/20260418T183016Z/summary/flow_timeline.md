...
│     OQS_SIG_sign → 3309-byte signature                        │  │  279.33 µs
└───────────────────────────────┬───────────────────────────────┘ ─┘
                                ▼
...
│         → CanIf_Transmit(fragment[i])                         │ │    2.10 µs
└───────────────────────────────┬───────────────────────────────┘ ─┘
                                ▼                                ─┐
┌───────────────────────────────────────────────────────────────┐ │
...
End-to-end (app Tx → app Rx): 420.15 µs
```
...
|----------------------------------------------|---------------|-------------|------|
| Com + PduR + SecOC header + FVM (residual)    |          0.40 | ≤       200 | PASS |
| Csm + CryIf + PQC + liboqs sign               |        279.33 | ≤       500 | PASS |
| CanTp / SoAd fragmentation                    |          2.10 | ≤       300 | PASS |
| CanIf / EthIf / driver + physical bus transit |        138.32 | ≤      2000 | PASS |
| Csm + CryIf + PQC + liboqs verify             |          0.00 | ≤       500 | PASS |
| End-to-end (application → application)        |        420.15 | ≤      5000 | PASS |
